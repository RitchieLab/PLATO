...
//added this no-parameter constructor
Database::Database()
:database("default.db")
,m_errhandler(NULL)
,m_mutex(m_mutex)
...
{
	Query myQuery(*this);
	if(!myQuery.execute("BEGIN;"))
	{
		//error handling
		std::cout<<"Could not open transaction\n";
	}
}
...
{
	Query myQuery(*this);
	if(!myQuery.execute("COMMIT;"))
	{
		//error handling
		std::cout<<"Could not commit transaction\n";
	}
}
...
{
	std::cout<<"Creating Lock in Database.cpp \n";
	Lock lck(m_mutex, m_b_use_mutex);
	std::cout<<"Finished Creating Lock in Database.cpp \n";
	OPENDB *odb = NULL;
