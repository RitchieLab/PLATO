...
,m_num_cols(0)
{
	std::cout << "In Query(Database&) Cosntructor\n";
}
...
,m_num_cols(0)
{
	std::cout << "In Query(Database&, String&) Constructor\n";
	execute(sql);
}
...
int Query::getColumnIndex(const std::string& col)
{
	if (m_nmap[col] >=0)
	{
		return m_nmap[col] - 1;
