...
,m_b_use_mutex(false)
{
	std::cout << "In Database Constructor 1 \n";
}
...
,m_b_use_mutex(true)
{
	std::cout << "In Database Constructor 2 \n";
}


Database::~Database()
{
	std::cout << "In Database Destructor \n";
...
void Database::freedb(Database::OPENDB *odb)
{
	...
	//testing to see if this might solve the error I'm getting about
	//destroying the database object before the query object...
	for (opendb_v::iterator it = m_opendbs.begin(); it != m_opendbs.end(); it++)
	{
		OPENDB *odb = NULL;
		odb = *it;
...
{
	std::cout << "In Database::transaction() \n";
	Query q(*this);
	if (!q.execute("BEGIN"))
	{
		return false;
