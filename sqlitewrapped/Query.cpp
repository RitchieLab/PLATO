...
*/
bool Query::executeb(const std::string& sql)
{
	std::cout<<"In Query::execute\n";
	...
	if (odb && !res)
	{
		...
		case SQLITE_BUSY:
			...
			return false;
		case SQLITE_DONE:
			std::cout<<"Finished Query::execute\n";
		case SQLITE_ROW:
			std::cout<<"Finished Query::execute\n";
			return true;
		case SQLITE_ERROR:
...
//new execute method to use sqlite3_exec() instead of _prepare and _step
bool Query::execute(const std::string & sql)
{
	...
	{
		const char *s = NULL;
		int rc = sqlite3_exec(odb -> db, sql.c_str(), NULL, 0, &db_error);
		...
		if (rc != SQLITE_OK)
		{
			...
			GetDatabase().error(*this, "execute: unknown error code");
		}
		return true;
	}
	return false;
...
int Query::getColumnIndex(std::string colName)
{
	int index = m_nmap[colName];
	if (index >=0)
	{
		return index; 
	}
