...
void HaploPhase::phaseAllHaplotypes()
{
  ...
  if ( opts::_haplo_plem_follow_ )
    {
      opts::_haplo_plem_follow_ = false;
      for (int i=0; i<samples->size(); i++)
	{
	  if ( (*samples)[i]->getFamID() == opts::_haplo_plem_follow_fid_ &&
	       (*samples)[i]->getInd() == opts::_haplo_plem_follow_iid_ )
	    {
			opts::printLog("Following individual [ " 
			 + opts::_haplo_plem_follow_fid_ + " " 
			 + opts::_haplo_plem_follow_iid_ + " ] in EM\n");
	      opts::_haplo_plem_follow_ind_ = i;
	      opts::_haplo_plem_follow_ = true;
	    }
	}
    }
  ...
  for (int l=0; l<nms; l++)
	...
	//cout << l+1<< " out of "<< nms 
	  ...
	  if(new_map[l]->getChrom() == opts::_CHRX_){
		  X = true;
	  }
      ...
      if (!same)
	{
	  ...
	  for (int i=0; i<samples->size(); i++)
	    {
	      Sample * person = (*samples)[i];
	      if (person->isFounder())
		{
		  if (haploid || (X && person->getSex()))
		    validN++;
		  else
		    validN+=2;
		}
...
//	    VPHASE.open("phased.verbose",ios::out);
	  ...
	  nsh = (int) pow(2.0, (double) opts::_haplo_plem_overlap_ );


	  // Check we have a valid overlap size
	  if ( opts::_haplo_plem_overlap_ >= opts::_haplo_plem_window_ )
	    opts::_haplo_plem_overlap_ = 0;


	  while (1)
	    {

	      HaploWindow * window = new HaploWindow( this , markers, samples );
	      
	      //////////////////////////////////
	      // Define SNP span for this window

	      window->start = pos;
	      window->stop = pos + opts::_haplo_plem_window_ - 1;
		...
		break;
	      ...
	      pos += opts::_haplo_plem_window_ - opts::_haplo_plem_overlap_;
	  ...
	  if (nonfounders)
	    {
	      ...
	      phasemap.clear();
	      phasemap.resize(samples->size());

	      if (opts::_test_hap_TDT_ || opts::_proxy_TDT_ )
		{
		  trans.clear();
		  untrans.clear();
		  trans.resize(nh,0);
	      ...
	      if (haploid ){
			  opts::printLog("Family-based haplotyping only for autosomes and X chromosome");
			  exit(1);
		  }
...
//	      vector_t dummy = performHaplotypeTests();
...
void HaploPhase::includeIndividuals(int i)
{
  ...
  // Do not look at non-reference individuals in some 
  // circumstances
