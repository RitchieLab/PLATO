...
earth::earth(){
	VERSION    = "version 1.4"; // change if you modify this file!
	BX_TOL      = 0.01;
	QR_TOL      = 0.01;
	MIN_GRSQ    = -10.0;
	ALMOST_ZERO = 1e-10;
...
int earth::GetMinSpan(int nCases, const double *bx,
...
int earth::GetEndSpan(const int nCases)
{
...
//-----------------------------------------------------------------------------
void earth::PrintForwardProlog(const int nCases,
    ...
    for (int iTerm = 1; iTerm < nTerms; iTerm++)
        if (UsedCols[iTerm]) {
            ...
            if (fabs(Betas_(iBestTerm, iResp)) >= MinBeta) {
					//%+-9.3g
            	sprintf(result, "%+9.3g", Betas_(iBestTerm, iResp));
            	best_model += " " + string(result);

            	printf("%+-9.3g", Betas_(iBestTerm, iResp));

//                best_model += Betas_(iBestTerm, iResp);
                nKnots = 0;
                for (int iPred = 0; iPred < nPreds; iPred++) {
                    switch(Dirs_(iTerm, iPred)) {
                        ...
                        case -1:
							...
							sprintf(result, s, Cuts_(iTerm, iPred), iPred);
							best_model += " " + string(result);
                        ...
                        case  1:
							...
							sprintf(result, s, iPred, Cuts_(iTerm, iPred));
							best_model += " " + string(result);
//							best_model += s;
...
//						best_model += s;
                ...
                //best_model += " // " + getString<int>(iBestTerm) + "\n";
            }
        }
		best_model += ")";
}
...
void earth::calculate(vector<int> loci, vector<int> covs){
    ...
    const int    nPreds = loci.size() + covs.size();//1;
