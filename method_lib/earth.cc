...
void* earth::calloc1(size_t num, size_t size)
{
...
earth::earth(){
	...
	MIN_GRSQ    = -10.0;
	ALMOST_ZERO = 1e-10;
	ONE         = 1;        // parameter for BLAS routines
...
void earth::PrintSortedQ(int nFastK)     // for debugging
{
#if _DEBUG_
...
int earth::CompareQ(const void *p1, const void *p2)     // for qsort
{
...
int earth::CompareAgedQ(const void *p1, const void *p2) // for qsort
{
...
int earth::Compare(const void *p1, const void *p2)  // for qsort
{
    const int i1 = *(int *)p1;
    const int i2 = *(int *)p2;
...
void earth::UpdateRssDeltaInQ(const int iParent, const int nTermsForRssDelta,
...
int earth::GetNextParent(   // returns -1 if no more parents
...
int* earth::OrderArray(const double x[], const int nRows, const int nCols)
{
...
int earth::GetNbrUsedCols(const bool UsedCols[], const int nLen)
{
    int nTrue = 0;
...
int earth::CopyUsedCols(double **pxUsed,                // out
...
//-----------------------------------------------------------------------------
double earth::Mean(const double x[], int n)
{
    double mean = 0;
    for (int i = 0; i < n; i++)
        mean += x[i] / n;
...
double earth::SumOfSquares(const double x[], const double mean, int n)
{
    double ss = 0;
    for (int i = 0; i < n; i++)
        ss += sq(x[i] - mean);
...
//-----------------------------------------------------------------------------
double earth::GetGcv(const int nTerms, // nbr basis terms including intercept
...
int earth::GetEndSpan(const int nCases, const int nPreds)
{
...
bool earth::GetNewFormFlag(const int iPred, const int iTerm,
...
//-----------------------------------------------------------------------------
double earth::GetCut(int iCase, const int iPred, const int nCases,
...
void earth::InitBetaCache(const bool UseBetaCache,
...
void earth::FindKnot(
    ...
    //???? added the next two to match with v 2.4
    const int nMinSpan,
    const int nEndSpan,
...
//-----------------------------------------------------------------------------
void earth::CheckVec(const double x[], int nCases, int nCols, const char sVecName[])
{
    int iCol, iCase;
...
//-----------------------------------------------------------------------------
void earth::PrintEstimatedMemoryUse(int nMaxTerms, int nCases, int nPreds, int nResp)
{
...
//-----------------------------------------------------------------------------
void earth::CheckRssNull(double RssNull, const double y[], int iResp, int nCases)
{
...
//-----------------------------------------------------------------------------
double* earth::pInitWeights(const double WeightsArg[], int nCases)
{
...
void earth::EvalSubsetsUsingXtx(
    ...
    const double Penalty)   // in: GCV penalty per knot
{
    ...
    //TODO:  add code to reduce the size of the PruneTerms matrix to nprune X nprune as in the R version
    //use realloc() to chop off the items from RssVec and columns from PruneTerms corresponding to a greater number of terms than we want to use...
    ...
    for (int iModel = 0; iModel < maxCount; iModel++)
    {
...
bool earth::checkForMissing(Sample* samp, vector<int> loci, vector<int> covs){
	for(unsigned int i = 0; i < loci.size(); i++){
		Marker* mark = data_set->get_locus(loci[i]);
		int mloc = mark->getLoc();
		if(samp->getAmissing(mloc)){
			return true;
		}
	}
	...
	for(unsigned int i = 0; i < covs.size(); i++){
		StepOptions mopts;
		mopts.getCovarMissing();
		if(samp->getCovariate(covs[i]) == data_set->get_missing_covalue()){
			return true;
		}
	}
...
void earth::calculate(vector<int> loci, vector<int> covs){
    const int    nMaxTerms = options->getMarsMaxTerms();
    int    nCases = 0;
    const int    nResp = 1;     // number of y columns
    int    nPreds = loci.size() + covs.size();//1; //const
    const int    nMaxDegree = options->getMarsDegree();
    const double Penalty = (nMaxDegree > 1)? 3: 2;
    const double Thresh = .001;
    const int    nMinSpan = 0;
    const bool   Prune = true;//true;
    const int    nFastK = 20;
    ...
    */ //end commented out for direct file read
    ...
    if(!in){
    	opts::printLog("Error opening test file: " + file + "\n");
    	//exit(1);
    	throw MethodException("Error opening test file: " + file + "\n");
    }
    string line = "";
    ...
    for(int i = 0; i < lines.size(); i++){
    	vector<string> tokes = General::ParseDelimitedLine(lines[i]);
    	for(int j = 0; j < tokes.size(); j++){
    		if(j == 0){
    			y[i] = atoi(tokes[j].c_str());
    		}
    		else{
    			if(tokes[j] != "" && tokes[j] != " "){
    				x[count++] = atoi(tokes[j].c_str());
    			}
    		}
    	}
    }
...
*/ //end commented out for direct file read
    ...
    cout <<"nPreds: " << getString<int>(nPreds) << "\n";
...
int earth::runMe(void)
{
    ...
    for (int i = 0; i < nCases; i++) {
    	y[i] = (rand() % 2);
    	outputFile << getString<double>(y[i]) << " ";
    	for (int j = 0; j < nPreds; j++)
    	{
    		x[count] = (rand() % 3);
    		outputFile << getString<double>(x[count]) << " ";
    		count++;
    	}
    	outputFile << "\n";
...
//        y[i] = 2;//sin(4 * x0);     // target function, change this to whatever you want
    ...
    //temporary loop to display the contents of the x and y matrices...
    for (int i = 0; i < nCases; i++)
    {
    	cout << "CASE NUMBER: " << getString<int>(i + 1) << "\n";
    	cout << getString<int>(y[i]);
    	for (int j = 0; j < nPreds; j++)
    	{
    		cout << " " << getString<int>(x[i * nPreds + j]);
    	}
    	cout << "\n";
    }
