...
////	bool map_done = false;
	...
	for(int m = 0; m < msize; m++){
		...
		if(mark->isEnabled() && isValidMarker(mark, &options, prev_base, prev_chrom)){
			...
			mdr_log << mark->toString() << "\tDisabled or doesn't meet chrom/bploc restrictions" << endl;
		}
	}
	prev_base = 0;
	prev_chrom = -1;
...
//						}
	...
	for(int i = 0; i < ssize; i++){
		Sample* samp = (*samples)[i];
		if((samp->isEnabled() ||(samp->isExcluded() && options.doIncExcludedSamples())) && !samp->isFlagged()){
			if(options.getMDRPedigreeOutput()){
				mdr << samp->getFamID() << "\t";
				mdr << samp->getInd() << "\t";
				mdr << samp->getDadID() << "\t";
