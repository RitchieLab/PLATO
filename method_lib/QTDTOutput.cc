				...
				if(mark->isEnabled()){
					...
					if(!mark->isMicroSat()){
						if(!samp->getAone(m_loc) && !samp->getAtwo(m_loc)){
							qtdt << "\t" << Helpers::map_allele(mark, mark->getAllele1(), &options) << "/" << Helpers::map_allele(mark, mark->getAllele1(), &options);
						}
						else if(!samp->getAone(m_loc) && samp->getAtwo(m_loc)){
							if(mark->getAllele1() > mark->getAllele2()){
								qtdt << "\t" << Helpers::map_allele(mark, mark->getAllele2(), &options) << "/" << Helpers::map_allele(mark, mark->getAllele1(), &options);
							}
							else{
								qtdt << "\t" << Helpers::map_allele(mark, mark->getAllele1(), &options) << "/" << Helpers::map_allele(mark, mark->getAllele2(), &options);
							}
						}
						else if(samp->getAone(m_loc) && samp->getAtwo(m_loc) && !samp->getAmissing(m_loc)){
							qtdt << "\t" << Helpers::map_allele(mark, mark->getAllele2(), &options) << "/" << Helpers::map_allele(mark, mark->getAllele2(), &options);
						}
						else if(samp->getAone(m_loc) && samp->getAtwo(m_loc) && samp->getAmissing(m_loc)){
							qtdt << "\t0/0";
						}
						...
						if(samp->getAbone(m_loc) != -1){
							if(mark->getAllele(samp->getAbone(m_loc)) < mark->getAllele(samp->getAbtwo(m_loc))){
								qtdt << "\t" << Helpers::map_allele(mark, mark->getAllele(samp->getAbone(m_loc)), &options) << "/" << Helpers::map_allele(mark, mark->getAllele(samp->getAbtwo(m_loc)), &options);
							}
							else{
								qtdt << "\t" << Helpers::map_allele(mark, mark->getAllele(samp->getAbtwo(m_loc)), &options) << "/" << Helpers::map_allele(mark, mark->getAllele(samp->getAbone(m_loc)), &options);
							}
						}
						else{
							qtdt << "\t0/0";
