...
namespace Methods{
class STRUCTOutput{// : public Process{
	private:
		DataSet* data_set;
...
//		void process(Connection*, Families*, Markers*);
		...
		void setThreshold(string s){
			options.setUp(s);
			//cout << "options.haveStratification B = " << options.haveStratification() << "\n";
		...
		void setOptions(StepOptions o){
			options = o;
			//cout << "options.haveStratification A = " << options.haveStratification() << "\n";
			if(options.getStratFile().length() == 0 && !(options.haveStratification())){
