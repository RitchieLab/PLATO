...
void Model::setMissing()
{
  ...
  for (int i=0; i<data_set->num_inds(); i++)//P->n; i++)
	  if(!data_set->get_sample(i)->isEnabled() || data_set->get_sample(i)->isFlagged()) miss[i] = true;
...
void Model::setMissing(vector<bool> & include)
{
  ...
  for (int i=0; i<data_set->num_inds(); i++)
    if(!data_set->get_sample(i)->isEnabled() || !include[i]) miss[i] = true;
...
void Model::addAdditiveSNP(int a)
{
  ...
  if ( opts::_CHRX_ == data_set->get_locus(a)->getChrom())//par::chr_sex[P->locus[a]->chr] )
  {  xchr.push_back(true); }//cout << "X CHR FOUND!!!!\n";}
  else
    xchr.push_back(false);

  if ( opts::_CHRXY_ == data_set->get_locus(a)->getChrom())//par::chr_haploid[P->locus[a]->chr] )
