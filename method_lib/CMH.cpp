...
///			while (person != sample.end()) {
			...
			for(int s = 0; s < data_set->num_inds(); s++){
				...
				if (!person->isEnabled() || person->isFlagged()) {
					// Next person
//					person++;
//					i1++;
//					i2++;
...
//		while (gperson != sample.end()) {
			...
			if (pperson->getAffected() && pperson->isEnabled() && !pperson->isFlagged()) {
			...
			} else if (pperson->isEnabled() && !pperson->isFlagged())//!pperson->missing) // Unaffecteds
			{
