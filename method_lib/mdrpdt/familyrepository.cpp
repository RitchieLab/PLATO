...
void PedigreeRepository::LoadDat(const char *filename) {
	...
	snpLabels.clear();
	int widestLabel = PdtModel::MaxLabelLength;
	while (!file.eof()) {
		string descriptor="", label="";
		file>>descriptor>>label;

		if ((int)label.length() > widestLabel)
...
// 	}
//
// cout << "done with ped reconcile" << endl;
// 	char label[64];
// 	int maxWidth = PdtModel::MaxLabelLength;
