...
PdtModel FoldEvaluation::ReportBestModel(std::ostream& os, int rank, int foldCount, Distribution::PTestDistribution *distribution) {
	...
	float pValue = distribution->Evaluate(bestModel.model.ModelSize()-1,bestModel.AvgMOR());
	float sumMOR = 0.0;
	...
	if (foldCount > 1) {
		for (int f=0; f<foldCount; f++) {
			...
			os<<setw(8)<<" "<<setw(width-1)<<" "<<" ";		//Rank is ignored for the individual slices / CVC
			os<<setw(labelWidth)<<model.GetPrintableLabel();
			os<<setw(width)<<setprecision(3)<<model.GetTrainingT(f);
			os<<setw(width)<<setprecision(3)<<model.GetTestingT(f);
			MatchedOddsRatio mor = model.EvaluateMOR(f);
			os<<setw(width)<<setprecision(3)<<mor.GetRatio();
			sumMOR += mor.GetRatio();
...
//			os<<setw(width)<<setprecision(3)<<model.EvaluateBiasedMOR(f).GetRatio();
	...
	if (distribution) 
		os<<setw(width)<<setprecision(ptestSigDigits)<<distribution->Evaluate(bestModel.model.ModelSize()-1,bestModel.AvgMOR());
	os<<"\n\n";
	return bestModel.model;	
}

FoldStatistic::FoldStatistic() : foldCount(0), cvc(0.0), sumTrainingT(0.0), sumTestingT(0.0) { }

FoldStatistic::FoldStatistic(int xvCount) : foldCount(xvCount), cvc(0.0), sumTrainingT(0.0), sumTestingT(0.0) { }
...
bool FoldStatistic::operator<(FoldStatistic& other) {
	if (cvc < other.cvc)
		return true;
	return cvc == other.cvc && AvgMOR() < other.AvgMOR();
