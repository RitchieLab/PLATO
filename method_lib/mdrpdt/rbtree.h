...
template <class K, class D, typename C > class RBTreeNode
{
    public:
        ...
        void    SetData(const D& newdata)   { std::cout<<"SetData()\n"; Data = newdata; }
...
template <class K, class D, typename C >
inline
RBTree<K,D,C>::RBTree(uint maxSize) : mCount(0), maxSize(maxSize)
{
    NIL = new RBTreeNode<K,D,C>;
    InitNode(NIL);
    NIL->Color = scBLACK;
...
RBTreeNode<K,D,C> *  RBTree<K,D,C>::Insert(const K &key, const D &data)
{
	...
	/** 
	 * @todo On Insert where maxSize > 0, I'm using FindMin() to check wether or not
	 ...
	 *
	 * @NOTE I've switched this to use key to determine when to drop 	
	 * 	a report off in the case of having a full tree. This needs to be
	 *  tested with other uses, prior to checking in (and this note removed)
	 */
...
template <class K, class D, typename C>
inline
int             RBTree<K,D,C>::GetCount() const
{
    return mCount;
}
