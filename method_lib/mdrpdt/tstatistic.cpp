...
/// Added for method library -- preps model before calling Evaluate Model
void TStatistic::EvaluateModel(GenotypeRepository *rep, PdtModel& currModel) {

	snpCount = rep->GetSnpCount();
	folds = rep->GetFolds();
	pedIDs = rep->GetPedigreeIDs();
	
	currModel.InitPedigree(indCount, pedCount, pedIDs);
	currModel.CrossValidations(xvCount, folds);	
	
	EvaluateModel(currModel);
