				...
				if(mark->isEnabled() && isValidMarker(mark, &options, prev_base, prev_chrom)){
					...
					if(!mark->isMicroSat()){
						if(!samp->getAone(m_loc) && !samp->getAtwo(m_loc)){
							fbat << " " << map_allele(mark, mark->getAllele1(), &options) << " " << map_allele(mark, mark->getAllele1(), &options);
						}
						else if(!samp->getAone(m_loc) && samp->getAtwo(m_loc)){
							if(mark->getAllele1() > mark->getAllele2()){
								fbat << " " << map_allele(mark, mark->getAllele2(), &options) << " " << map_allele(mark, mark->getAllele1(), &options);
							}
							else{
								fbat << " " << map_allele(mark, mark->getAllele1(), &options) << " " << map_allele(mark, mark->getAllele2(), &options);
							}
						}
						else if(samp->getAone(m_loc) && samp->getAtwo(m_loc) && !samp->getAmissing(m_loc)){
							fbat << " " << map_allele(mark, mark->getAllele2(), &options) << " " << map_allele(mark, mark->getAllele2(), &options);
						}
						else if(samp->getAone(m_loc) && samp->getAtwo(m_loc) && samp->getAmissing(m_loc)){
							fbat << " 0 0";
						}
						...
						if(samp->getAbone(m_loc) != -1){
							if(mark->getAllele(samp->getAbone(m_loc)) < mark->getAllele(samp->getAbtwo(m_loc))){
								fbat << " " << map_allele(mark, mark->getAllele(samp->getAbone(m_loc)), &options) << " " << map_allele(mark, mark->getAllele(samp->getAbtwo(m_loc)), &options);
							}
							else{
								fbat << " " << map_allele(mark, mark->getAllele(samp->getAbtwo(m_loc)), &options) << " " << map_allele(mark, mark->getAllele(samp->getAbone(m_loc)), &options);
							}
						}
						else{
							fbat << " 0 0";
