#ifndef CLUSTERMISSING_H
#define CLUSTERMISSING_H
...
class ClusterMissing{
	...
	public:
		...
		ClusterMissing(float thresh) : threshold(thresh){
			families = NULL;
			markers = NULL;
			samples = NULL;
			orig_num_markers = 0;
		...
		void filter();
		void filterOne(int);
		void setOptions(StepOptions opts){
			options = opts;
		}
		...
		int calcPairLocus(int s1, int s2, int m);
		...
		double cldist(vector<vector<double> > & d,
		          vector<int> & a,
		          vector<int> & b);
		double calcGenomeIBM(Sample* one, Sample* two);
};
};
#endif
