...
//						}
						...
						if(!map_done){
							grr_map << mark->getChrom() << "\t" << mark->getBPLOC() << "\t" << mark->getRSID() << "\t" << map_allele(mark, mark->getAllele1(), &options) << "/" << map_allele(mark, mark->getAllele2(), &options) << endl;
						}
						...
						if(!mark->isMicroSat()){
							if(!samp->getAone(m_loc) && !samp->getAtwo(m_loc)){
								grr << "\t" << map_allele(mark, mark->getAllele1(), &options) << "/" << map_allele(mark, mark->getAllele1(), &options);
							}
							else if(!samp->getAone(m_loc) && samp->getAtwo(m_loc)){
								if(mark->getAllele1() > mark->getAllele2()){
									grr << "\t" << map_allele(mark, mark->getAllele2(), &options) << "/" << map_allele(mark, mark->getAllele1(), &options);
								}
								else{
									grr << "\t" << map_allele(mark, mark->getAllele1(), &options) << "/" << map_allele(mark, mark->getAllele2(), &options);
								}
							}
							else if(samp->getAone(m_loc) && samp->getAtwo(m_loc) && !samp->getAmissing(m_loc)){
								grr << "\t" << map_allele(mark, mark->getAllele2(), &options) << "/" << map_allele(mark, mark->getAllele2(), &options);
							}
							else if(samp->getAone(m_loc) && samp->getAtwo(m_loc) && samp->getAmissing(m_loc)){
								grr << "\t0/0";
							}
							...
							if(samp->getAbone(m_loc) != -1){
								if(mark->getAllele(samp->getAbone(m_loc)) < mark->getAllele(samp->getAbtwo(m_loc))){
									grr << "\t" << map_allele(mark, mark->getAllele(samp->getAbone(m_loc)), &options) << "/" << map_allele(mark, mark->getAllele(samp->getAbtwo(m_loc)), &options);
								}
								else{
									grr << "\t" << map_allele(mark, mark->getAllele(samp->getAbtwo(m_loc)), &options) << "/" << map_allele(mark, mark->getAllele(samp->getAbone(m_loc)), &options);
								}
							}
							else{
								grr << "\t0/0";
