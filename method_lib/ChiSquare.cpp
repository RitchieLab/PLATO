...
///@return p value
///
double ChiSquare::pfromchi(double x, int df){
  double  a, s, y=0;
  double  e, c, z;
...
/// Ret: chi square
///
double ChiSquare::chisquare(vector<vector<unsigned int> > & chiTotals, int * df){
  int affected=0, unaffected=0;
  *df = -1; // in this case degrees of freedom will be one less then
...
/// @return chi square
///
// double ChiSquare::chisquare(vector<vector<float> > & chiTotals, int * df){
//   float affected=0.0, unaffected=0.0;
//   *df = -1; // in this case degrees of freedom will be one less then
//             // the number of cells in the penetrance table
//             // This assumes the individuals have only two states
//   vector<float> totalInCell;
//   for(unsigned int cell=0; cell<chiTotals[0].size(); ++cell){
//     unaffected+=chiTotals[0][cell];
//     affected+=chiTotals[1][cell];
//     totalInCell.push_back(chiTotals[0][cell] + chiTotals[1][cell]);
//   }
// 
//   double totalinds = unaffected + affected;
//   double chiscore=0.0, unaffexpected, affexpected;
//   // calculate expected for each cell
//   for(unsigned int cell=0; cell<chiTotals[0].size(); ++cell){
//     if( totalInCell[cell] > 0){
//       ++(*df); // increment degrees of freedom when 
//       unaffexpected = totalInCell[cell] * unaffected / totalinds;
//       affexpected = totalInCell[cell] * affected / totalinds;
//       chiscore += pow((chiTotals[0][cell]-unaffexpected),2) / unaffexpected;
//       chiscore += pow((chiTotals[1][cell]-affexpected),2) / affexpected;
//     }
//   }
...
///
/// Calculate chi square <br>
/// Assumes binary status with each individual being either a 0 or 1.<br>
/// Only includes indexes passed as arguments so certain cells
/// can be skipped as when ignoring missing data in a set
/// @param chiTotals two dimensional vector with each row being a status and
///        each column a genotype.  Values represent totals for
///        that status and genotype
/// @param includedIndexes vector listing indexes to include in
///        the calculation
/// @param df pointer to integer that will hold degrees of freedom for this
///        model -- it is an output argument
/// Ret: chi square
...
//             // This assumes the individuals have only two states
//   vector<int> totalInCell;
//   for(unsigned int cell=0; cell<includedIndexes.size(); cell++){
//     unaffected+=chiTotals[0][includedIndexes[cell]];
//     affected+=chiTotals[1][includedIndexes[cell]];
//     totalInCell.push_back(chiTotals[0][includedIndexes[cell]] 
//       + chiTotals[1][includedIndexes[cell]]);
//   }
// 
//   double totalinds = double(unaffected) + affected;
//   double chiscore=0.0;
//   // calculate expected for each cell
//   for(unsigned int cell=0; cell<totalInCell.size(); cell++){
//     if( totalInCell[cell] > 0){
//       (*df)++; // increment degrees of freedom when
//       double unaffexpected = double(totalInCell[cell]) * unaffected / totalinds;
//       double affexpected = double(totalInCell[cell]) * affected / totalinds;
//       chiscore += pow((chiTotals[0][includedIndexes[cell]]-unaffexpected),2)
//         / unaffexpected;
//       chiscore += pow((chiTotals[1][includedIndexes[cell]]-affexpected),2) 
//         / affexpected;
//     }
//   }
//   return chiscore;
...
///
int ChiSquare::calcdf(int numloci, int numLociValues){
