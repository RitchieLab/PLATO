...
//	}
	...
	if(!alldigit){
		...
		printFamsToDigit(families, "input_lapis", options);
	}
	...
	for(int m = 0; m < msize; m++){
		...
		if(mark->isEnabled()){// && !mark->isFlagged()){
			...
			if(mark->getNumAlleles() < 3){
				...
				str << map_allele(mark, mark->getAllele1(), &options) << " " << map_allele(mark, mark->getAllele2(), &options) << endl;
			}
			else{
				...
				str << endl;
				str << map_allele(mark, mark->getAllele(0), &options);
				for(int a = 1; a < mark->getNumAlleles(); a++){
					str << " " << map_allele(mark, mark->getAllele(a), &options);
				}
				str << endl;
			}
			str << endl;
