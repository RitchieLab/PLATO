...
void MendelianErrors::perform_evaluation(bool output){
	...
	while(!overwrite && fileExists(fname)){
	...
	while(!overwrite && fileExists(fname2)){
		fname2 += "." + getString<int>(order);
	}
				...
				if((*samples)[s]->isEnabled()){
					...
					if((*samples)[s]->getDadID() != "0" && (*samples)[s]->getDad() != NULL){
						if(((*samples)[s]->getAone(mloc) && (*samples)[s]->getAtwo(mloc) && (*samples)[s]->getAmissing(mloc) &&
							!(*markers)[m]->isMicroSat()) ||
							((*samples)[s]->getDad()->getAone(mloc) && (*samples)[s]->getDad()->getAtwo(mloc) && (*samples)[s]->getDad()->getAmissing(mloc) &&
							 !(*markers)[m]->isMicroSat()) ||
							((*markers)[m]->isMicroSat() && (*samples)[s]->getAbone(mloc) == -1) ||
							((*markers)[m]->isMicroSat() && (*samples)[s]->getDad()->getAbone(mloc) == -1)){
							//continue;
						}
						else{
							if((*markers)[m]->getChrom() != opts::_CHRX_ || (!(*samples)[s]->getSex() && (*markers)[m]->getChrom() == opts::_CHRX_)){
								if((!(*markers)[m]->isMicroSat() &&
									...
									if(!inc && found == family_inc.end()){
										...
										if(opts::_ENZYMES_){
											fenzyme[(*samples)[s]->getFamily()->getLoc()][(*markers)[m]->getEnzyme()]++;
										}
										merrors[m]++;
									...
									if(!dadinc){
										...
										if(opts::_ENZYMES_){
											senzyme[(*samples)[s]->getDad()->getLoc()][(*markers)[m]->getEnzyme()]++;
										}
										dadinc = true;
									}
									//cout << "we1\n";
									write_error(erroroutput, (*markers)[m], (*samples)[s], (*samples)[s]->getDad(), mloc);
								}
							}
						}//end else
					}
					...
					if((*samples)[s]->getMomID() != "0" && (*samples)[s]->getMom() != NULL){
						if((!(*markers)[m]->isMicroSat() &&
							...
							((*markers)[m]->isMicroSat() && ((*samples)[s]->getAbone(mloc) == -1 || (*samples)[s]->getAbone(mloc) == -1)) || ((*markers)[m]->isMicroSat() && ((*samples)[s]->getMom()->getAbone(mloc) == -1))){
							//continue;
						}
						else{
							if((!(*markers)[m]->isMicroSat() &&
								...
								if(!mominc){
									...
									if(opts::_ENZYMES_){
										senzyme[(*samples)[s]->getMom()->getLoc()][(*markers)[m]->getEnzyme()]++;
									}
									mominc = true;
								}
								//cout << "we2:" << (*markers)[m]->toString() << "\t" << (*samples)[s]->toString() << "\t" << (*markers)[m]->getLoc() << ":" << mloc << "\n";
								write_error(erroroutput, (*markers)[m], (*samples)[s], (*samples)[s]->getMom(), mloc);
							}
						}//end else
					}
					...
					if(childinc){
						...
						error_map[s].push_back(m);
					}
					if(dadinc){
						(*samples)[s]->getFamily()->setMeError(true);
						error_map[(*samples)[s]->getDad()->getLoc()].push_back(m);
					}
					if(mominc){
						(*samples)[s]->getFamily()->setMeError(true);
						error_map[(*samples)[s]->getMom()->getLoc()].push_back(m);
					}
				}
			}// end level 1
...
bool MendelianErrors::checkErrors(Family* fam, Marker* mark){
	...
	for(int i = 0; i < (int)samps->size(); i++){
		...
		for(int m = 0; m < (int)error_map[samp->getLoc()].size(); m++){
			if((*markers)[error_map[samp->getLoc()][m]] == mark){
				return false;
			}
		}
	}
