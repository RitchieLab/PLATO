	...
	for (int i = 0; i < data_set->num_inds(); i++)
		...
		if (!miss[i]) {
			...
			if (options.getUsePheno()) {
				...
				//cout << "Pushing " << getString<double>(data_set->get_sample(i)->getPheno(index)) << " onto Y\n";
			} else {
				Y.push_back(data_set->get_sample(i)->getPheno());//P->sample[i]->pperson->phenotype ) ;
				//cout << "Pushing " << getString<double>(data_set->get_sample(i)->getPheno()) << " onto Y\n";
			}
		}
}
...
//	cout << "R2: " << r2 << endl;
//	cout << "F: " << result << " on " << tempnp << " and " << (nind - tempnp - 1) << ". Pvalue: ";
