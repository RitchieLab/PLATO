	...
	if(options.doThreshMarkersLow() || options.doThreshMarkersHigh()){
		...
		for(int i = 0; i < size; i++){
			if((*markers).at(i)->isEnabled() && !(*markers).at(i)->isFlagged()){
				...
				bool inc = false;
				if(options.doThreshMarkersHigh() && Helpers::dGreater(pval.at(i), options.getThreshMarkersHigh())){
					(*markers).at(i)->setEnabled(false);
					inc = true;
				}
				if(options.doThreshMarkersLow() && Helpers::dLess(pval.at(i),options.getThreshMarkersLow())){
					(*markers).at(i)->setEnabled(false);
					inc = true;
				}
				if(inc){
...
void RunTDT::calculate(Marker* mark, vector<Sample*> gsamps){
	chi_one = 0;
	pval_one = 0;
	fams_used_one = 0;
	trans_one = 0;
	...
	if(!mark->isMicroSat()){
		...
		for(int i = 0; i < ssize; i++){
			Sample* samp = gsamps.at(i);
			Sample* dad = samp->getDad();
			Sample* mom = samp->getMom();
			if(dad != NULL && mom != NULL){
				vector<Sample*>::iterator diter = find_if(gsamps.begin(), gsamps.end(), FindSampleByFamAndID(dad->getFamID(), dad->getInd()));
				if(diter != gsamps.end()){
					vector<Sample*>::iterator miter = find_if(gsamps.begin(), gsamps.end(), FindSampleByFamAndID(mom->getFamID(), mom->getInd()));
					if(miter != gsamps.end()){
