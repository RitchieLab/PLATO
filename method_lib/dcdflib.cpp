...
static double algdiv_var,c,d,h,s11,s3,s5,s7,s9,t,u,v,w,x,x2,T1;
/*
     ..
...
static int K3 = 4;
static int K5 = 5;
static double alngam_var,offset,prod,xx;
static int i,n;
...
S50:
    T2 = xx-2.0e0;
    T4 = xx-2.0e0;
    alngam_var = devlpl(scoefn,&K1,&T2)/devlpl(scoefd,&K3,&T4);
/*
     COMPUTE RATIONAL APPROXIMATION TO GAMMA(X)
*/
    alngam_var *= prod;
    alngam_var = log(alngam_var);
    goto S110;
S70:
    offset = hln2pi;
/*
...
*/
    T6 = 1.0e0/pow(xx,2.0);
    alngam_var = devlpl(coef,&K5,&T6)/xx;
    alngam_var += (offset+(xx-0.5e0)*log(xx)-xx);
S110:
    return alngam_var;
#undef hln2pi
}
...
static double alnrel_var,t,t2,w,x;
/*
     ..
     .. Executable Statements ..
*/
...
static double apser_var,aj,bx,c,j,s,t,tol;
/*
     ..
     .. Executable Statements ..
*/
...
*/
static int K3 = 1;
static double basym_var,bsum,dsum,f,h,h2,hn,j0,j1,r,r0,r1,s,sum,t,t0,t1,u,w,w0,z,z0,
    z2,zn,znm1;
...
static double c4 = .837308034031215e-03;
static double c5 = -.165322962780713e-02;
static double bcorr_var,a,b,c,h,s11,s3,s5,s7,s9,t,w,x,x2;
/*
     ..
     .. Executable Statements ..
*/
...
*/
    t = pow(1.0e0/a,2.0);
    bcorr_var = (((((c5*t+c4)*t+c3)*t+c2)*t+c1)*t+c0)/a+w;
    return bcorr_var;
}
...
static double betaln_var,a,b,c,h,u,v,w,z;
static int i,n;
static double T1;
/*
     ..
...
*/
    if(b >= 8.0e0) goto S10;
    T1 = a+b;
    betaln_var = gamln(&a)+(gamln(&b)-gamln(&T1));
    return betaln_var;
S10:
    betaln_var = gamln(&a)+algdiv(&a,&b);
    return betaln_var;
S20:
/*
-----------------------------------------------------------------------
...
*/
    if(a > 2.0e0) goto S40;
    if(b > 2.0e0) goto S30;
    betaln_var = gamln(&a)+gamln(&b)-gsumln(&a,&b);
    return betaln_var;
S30:
    w = 0.0e0;
    if(b < 8.0e0) goto S60;
...
*/
    ...
    w = log(w);
    if(b < 8.0e0) goto S60;
    betaln_var = w+gamln(&a)+algdiv(&a,&b);
    return betaln_var;
S60:
/*
...
*/
    ...
    betaln_var = w+log(z)+(gamln(&a)+(gamln(&b)-gsumln(&a,&b)));
    return betaln_var;
S80:
/*
...
*/
    ...
    betaln_var = log(w)-(double)n*log(b)+(gamln(&a)+algdiv(&a,&b));
    return betaln_var;
S100:
/*
-----------------------------------------------------------------------
...
*/
    ...
    v = b*alnrel(&h);
    if(u <= v) goto S110;
    betaln_var = -(0.5e0*log(b))+e+w-v-u;
    return betaln_var;
S110:
    betaln_var = -(0.5e0*log(b))+e+w-u-v;
    return betaln_var;
}
...
*/
{
static double bfrac_var,alpha,an,anp1,beta,bn,bnp1,c,c0,c1,e,n,p,r,r0,s,t,w,yp1;
/*
     ..
     .. Executable Statements ..
*/
    bfrac_var = brcomp(a,b,x,y);
    if(bfrac_var == 0.0e0) return bfrac_var;
    c = 1.0e0+*lambda;
    c0 = *b/ *a;
    c1 = 1.0e0+1.0e0/ *a;
    yp1 = *y+1.0e0;
...
*/
{
static double bpser_var,a0,apb,b0,c,n,sum,t,tol,u,w,z;
static int i,m;
/*
     ..
     .. Executable Statements ..
...
*/
    bpser_var = pow(*x,*a);
    if(bpser_var == 0.0e0) return bpser_var;
    apb = *a+*b;
...
S70:
    ...
    t = (1.0e0+gam1(&u))/apb;
S80:
    bpser_var = exp(z)*(a0/ *a)*(1.0e0+gam1(&b0))/t;
    goto S100;
S90:
/*
...
*/
    ...
    z = *a*log(*x)-u;
    bpser_var = a0/ *a*exp(z);
S100:
    if(bpser_var == 0.0e0 || *a <= 0.1e0**eps) return bpser_var;
/*
-----------------------------------------------------------------------
                     COMPUTE THE SERIES
-----------------------------------------------------------------------
...
static double brcmp1_var,a0,apb,b0,c,e,h,lambda,lnx,lny,t,u,v,x0,y0,z;
static int i,n;
/*
-----------------
     CONST = 1/SQRT(2*PI)
...
*/
    brcmp1_var = esum(mu,&z);
    if(brcmp1_var == 0.0e0) return brcmp1_var;
    apb = *a+*b;
...
S60:
    c = (1.0e0+gam1(a))*(1.0e0+gam1(b))/z;
    brcmp1_var = brcmp1_var*(a0*c)/(1.0e0+a0/b0);
    return brcmp1_var;
S70:
/*
...
S100:
    ...
    t = (1.0e0+gam1(&u))/apb;
S110:
    brcmp1_var = a0*esum(mu,&z)*(1.0e0+gam1(&b0))/t;
    return brcmp1_var;
S120:
/*
...
S190:
    T4 = -(*a*u+*b*v);
    z = esum(mu,&T4);
    brcmp1_var = Const*sqrt(*b*x0)*z*exp(-bcorr(a,b));
    return brcmp1_var;
}
...
{
static double Const = .398942280401433e0;
static double brcomp_var,a0,apb,b0,c,e,h,lambda,lnx,lny,t,u,v,x0,y0,z;
static int i,n;
/*
-----------------
     CONST = 1/SQRT(2*PI)
...
*/
    brcomp_var = 0.0e0;
    if(*x == 0.0e0 || *y == 0.0e0) return brcomp_var;
    a0 = fifdmin1(*a,*b);
    if(a0 >= 8.0e0) goto S130;
    if(*x > 0.375e0) goto S10;
    lnx = log(*x);
...
*/
    brcomp_var = exp(z);
    if(brcomp_var == 0.0e0) return brcomp_var;
    apb = *a+*b;
    if(apb > 1.0e0) goto S50;
    z = 1.0e0+gam1(&apb);
    goto S60;
...
S60:
    c = (1.0e0+gam1(a))*(1.0e0+gam1(b))/z;
    brcomp_var = brcomp_var*(a0*c)/(1.0e0+a0/b0);
    return brcomp_var;
S70:
/*
                ALGORITHM FOR 1 .LT. B0 .LT. 8
...
S100:
    ...
    t = (1.0e0+gam1(&u))/apb;
S110:
    brcomp_var = a0*exp(z)*(1.0e0+gam1(&b0))/t;
    return brcomp_var;
S120:
/*
                   ALGORITHM FOR B0 .GE. 8
...
S190:
    z = exp(-(*a*u+*b*v));
    brcomp_var = Const*sqrt(*b*x0)*z*exp(-bcorr(a,b));
    return brcomp_var;
}
...
static int K1 = 1;
static int K2 = 0;
static double bup_var,ap1,apb,d,l,r,t,w;
...
*/
    ...
    d = exp(-t);
S10:
    bup_var = brcmp1(&mu,a,b,x,y)/ *a;
    if(*n == 1 || bup_var == 0.0e0) return bup_var;
    nm1 = *n-1;
    w = d;
/*
...
*/
{
static double dbetrm_var,T1,T2,T3;
/*
     ..
     .. Executable Statements ..
*/
...
*/
    T1 = *a+*b;
    dbetrm_var = -dstrem(&T1);
    T2 = fifdmax1(*a,*b);
    dbetrm_var += dstrem(&T2);
    T3 = fifdmin1(*a,*b);
    dbetrm_var += dstrem(&T3);
    return dbetrm_var;
}
...
static double dexpm1_var,w;
/*
     ..
     .. Executable Statements ..
*/
    if(fabs(*x) > 0.15e0) goto S10;
    dexpm1_var = *x*(((p2**x+p1)**x+1.0e0)/((((q4**x+q3)**x+q2)**x+q1)**x+1.0e0));
    return dexpm1_var;
S10:
    w = exp(*x);
    if(*x > 0.0e0) goto S20;
    dexpm1_var = w-0.5e0-0.5e0;
    return dexpm1_var;
S20:
    dexpm1_var = w*(0.5e0+(0.5e0-1.0e0/w));
    return dexpm1_var;
}
...
static double dinvnr_var,strtx,xcur,cum,ccum,pp,dx;
static int i;
...
*/
    if(!qporq) dinvnr_var = -dinvnr_var;
    return dinvnr_var;
S40:
/*
...
static int K1 = 12;
static double dlanor_var,approx,correc,xx,xx2,T2;
...
*/
{
static double dln1mx_var,T1;
/*
     ..
     .. Executable Statements ..
*/
    T1 = -*x;
    dln1mx_var = dln1px(&T1);
    return dln1mx_var;
}
...
static double q2 = .747811014037616e+00;
static double q3 = -.845104217945565e-01;
static double dln1px_var,t,t2,w,x;
/*
     ..
     .. Executable Statements ..
*/
...
{
static double e = .918938533204673e0;
static double dlnbet_var,a,b,c,h,u,v,w,z;
static int i,n;
static double T1;
/*
     ..
...
*/
    if(b >= 8.0e0) goto S10;
    T1 = a+b;
    dlnbet_var = gamln(&a)+(gamln(&b)-gamln(&T1));
    return dlnbet_var;
S10:
    dlnbet_var = gamln(&a)+algdiv(&a,&b);
    return dlnbet_var;
S20:
/*
-----------------------------------------------------------------------
...
*/
    if(a > 2.0e0) goto S40;
    if(b > 2.0e0) goto S30;
    dlnbet_var = gamln(&a)+gamln(&b)-gsumln(&a,&b);
    return dlnbet_var;
S30:
    w = 0.0e0;
    if(b < 8.0e0) goto S60;
...
*/
    ...
    w = log(w);
    if(b < 8.0e0) goto S60;
    dlnbet_var = w+gamln(&a)+algdiv(&a,&b);
    return dlnbet_var;
S60:
/*
                 REDUCTION OF B WHEN B .LT. 8
*/
    ...
    dlnbet_var = w+log(z)+(gamln(&a)+(gamln(&b)-gsumln(&a,&b)));
    return dlnbet_var;
S80:
/*
                REDUCTION OF A WHEN B .GT. 1000
*/
    ...
    dlnbet_var = log(w)-(double)n*log(b)+(gamln(&a)+algdiv(&a,&b));
    return dlnbet_var;
S100:
/*
-----------------------------------------------------------------------
...
*/
    ...
    v = b*alnrel(&h);
    if(u <= v) goto S110;
    dlnbet_var = -(0.5e0*log(b))+e+w-v-u;
    return dlnbet_var;
S110:
    dlnbet_var = -(0.5e0*log(b))+e+w-u-v;
    return dlnbet_var;
};
/*
**********************************************************************
...
S20:
    ...
    T1 = t-1.0e0;
    dlngam_var = gamln1(&T1)+log(w);
    return dlngam_var;
S40:
    t = pow(1.0e0/ *a,2.0);
    w = (((((c5*t+c4)*t+c3)*t+c2)*t+c1)*t+c0)/ *a;
    dlngam_var = d+w+(*a-0.5e0)*(log(*a)-1.0e0);
    return dlngam_var;
};
...
static int K1 = 10;
static double dstrem_var,sterl,T2;
/*
     ..
     .. Executable Statements ..
*/
...
static double dt1_var,denpow,sum,term,x,xp,xx;
static int i;
/*
     ..
     .. Executable Statements ..
...
static double erf1_var,ax,bot,t,top,x2;
/*
     ..
     .. Executable Statements ..
*/
...
S10:
    ...
    bot = ((((((q[0]*ax+q[1])*ax+q[2])*ax+q[3])*ax+q[4])*ax+q[5])*ax+q[6])*ax+q[
      7];
    erf1_var = 0.5e0+(0.5e0-exp(-(*x**x))*top/bot);
    if(*x < 0.0e0) erf1_var = -erf1_var;
    return erf1_var;
S20:
    ...
    erf1_var = (c-top/(x2*bot))/ax;
    erf1_var = 0.5e0+(0.5e0-exp(-x2)*erf1_var);
    if(*x < 0.0e0) erf1_var = -erf1_var;
    return erf1_var;
S30:
    erf1_var = fifdsign(1.0e0,*x);
    return erf1_var;
}
...
static int K1 = 1;
static double erfc1_var,ax,bot,e,t,top,w;
/*
     ..
     .. Executable Statements ..
*/
...
*/
    ...
    top = (((a[0]*t+a[1])*t+a[2])*t+a[3])*t+a[4]+1.0e0;
    bot = ((b[0]*t+b[1])*t+b[2])*t+1.0e0;
    erfc1_var = 0.5e0+(0.5e0-*x*(top/bot));
    if(*ind != 0) erfc1_var = exp(t)*erfc1_var;
    return erfc1_var;
S10:
/*
                  0.5 .LT. ABS(X) .LE. 4
...
S30:
    ...
    top = (((r[0]*t+r[1])*t+r[2])*t+r[3])*t+r[4];
    bot = (((s[0]*t+s[1])*t+s[2])*t+s[3])*t+1.0e0;
    erfc1_var = (c-t*top/bot)/ax;
S40:
/*
                      FINAL ASSEMBLY
*/
    if(*ind == 0) goto S50;
    if(*x < 0.0e0) erfc1_var = 2.0e0*exp(*x**x)-erfc1_var;
    return erfc1_var;
S50:
    w = *x**x;
    t = w;
    e = w-t;
    erfc1_var = (0.5e0+(0.5e0-e))*exp(-t)*erfc1_var;
    if(*x < 0.0e0) erfc1_var = 2.0e0-erfc1_var;
    return erfc1_var;
S60:
/*
...
*/
    erfc1_var = 2.0e0;
    if(*ind != 0) erfc1_var = 2.0e0*exp(*x**x);
    return erfc1_var;
S70:
/*
...
S40:
    if(*l == 0) goto S50;
    m = ipmpar(&K2)-1;
    exparg_var = 0.99999e0*((double)m*lnb);
    return exparg_var;
S50:
    m = ipmpar(&K3);
    exparg_var = 0.99999e0*((double)m*lnb);
...
{
static int K1 = 1;
static double fpser_var,an,c,s,t,tol;
/*
     ..
     .. Executable Statements ..
*/
    ...
    fpser_var = 0.0e0;
    t = *a*log(*x);
    if(t < exparg(&K1)) return fpser_var;
    fpser_var = exp(t);
S10:
/*
                NOTE THAT 1/B(A,B) = B
*/
    fpser_var = *b/ *a*fpser_var;
    tol = *eps/ *a;
    an = *a+1.0e0;
    t = *x;
    s = t/an;
...
static double gam1_var,bot,d,t,top,w,T1;
/*
     ..
     .. Executable Statements ..
*/
...
S20:
    ...
    T1 = t-1.0e0;
    gamln_var = gamln1(&T1)+log(w);
    return gamln_var;
S40:
    t = pow(1.0e0/ *a,2.0);
    w = (((((c5*t+c4)*t+c3)*t+c2)*t+c1)*t+c0)/ *a;
    gamln_var = d+w+(*a-0.5e0)*(log(*a)-1.0e0);
    return gamln_var;
}
...
static int K2 = 3;
static int K3 = 0;
static double Xgamm_var,bot,g,lnx,s,t,top,w,x,z;
...
*/
    ...
    if(fabs(t)*spmpar(&K2) <= 1.0001e0) return Xgamm_var;
    Xgamm_var = 1.0e0/t;
    return Xgamm_var;
S80:
/*
...
-----------------------------------------------------------------------
*/
    if(fabs(*a) >= 1.e3) return Xgamm_var;
...
*/
    ...
    w = g;
    t = g-w;
    if(w > 0.99999e0*exparg(&K3)) return Xgamm_var;
    Xgamm_var = exp(w)*(1.0e0+t);
    if(*a < 0.0e0) Xgamm_var = 1.0e0/(Xgamm_var*s)/x;
    return Xgamm_var;
}
...
*/
{
static double gsumln_var,x,T1,T2;
/*
     ..
     .. Executable Statements ..
*/
    ...
    T1 = 1.0e0+x;
    gsumln_var = gamln1(&T1);
    return gsumln_var;
S10:
    if(x > 1.25e0) goto S20;
    gsumln_var = gamln1(&x)+alnrel(&x);
    return gsumln_var;
S20:
    T2 = x-1.0e0;
    gsumln_var = gamln1(&T2)+log(x*(1.0e0+x));
    return gsumln_var;
}
...
static int K1 = 3;
static int K2 = 1;
static double psi_var,aug,den,sgn,upper,w,x,xmax1,xmx0,xsmall,z;
static int i,m,n,nq;
/*
     ..
     .. Executable Statements ..
...
{
static double rt2pin = .398942280401433e0;
static double rcomp_var,t,t1,u;
/*
     ..
     .. Executable Statements ..
*/
    ...
    t = *a*log(*x)-*x;
    if(*a >= 1.0e0) goto S10;
    rcomp_var = *a*exp(t)*(1.0e0+gam1(a));
    return rcomp_var;
S10:
    rcomp_var = exp(t)/Xgamm(a);
    return rcomp_var;
S20:
    u = *x/ *a;
    if(u == 0.0e0) return rcomp_var;
    t = pow(1.0e0/ *a,2.0);
    t1 = (((0.75e0*t-1.0e0)*t+3.5e0)*t-105.0e0)/(*a*1260.0e0);
    t1 -= (*a*rlog(&u));
    rcomp_var = rt2pin*sqrt(*a)*exp(t1);
    return rcomp_var;
}
...
static double q3 = -.119041179760821e-01;
static double q4 = .595130811860248e-03;
static double rexp_var,w;
/*
     ..
     .. Executable Statements ..
*/
    if(fabs(*x) > 0.15e0) goto S10;
    rexp_var = *x*(((p2**x+p1)**x+1.0e0)/((((q4**x+q3)**x+q2)**x+q1)**x+1.0e0));
    return rexp_var;
S10:
    w = exp(*x);
    if(*x > 0.0e0) goto S20;
    rexp_var = w-0.5e0-0.5e0;
    return rexp_var;
S20:
    rexp_var = w*(0.5e0+(0.5e0-1.0e0/w));
    return rexp_var;
}
...
static double rlog_var,r,t,u,w,w1;
/*
     ..
     .. Executable Statements ..
*/
...
*/
    ...
    rlog_var = 2.0e0*t*(1.0e0/(1.0e0-r)-r*w)+w1;
    return rlog_var;
S40:
    r = *x-0.5e0-0.5e0;
    rlog_var = r-log(*x);
...
static double q1 = -.127408923933623e+01;
static double q2 = .354508718369557e+00;
static double rlog1_var,h,r,t,w,w1;
/*
     ..
     .. Executable Statements ..
*/
...
*/
    ...
    t = r*r;
    w = ((p2*t+p1)*t+p0)/((q2*t+q1)*t+1.0e0);
    rlog1_var = 2.0e0*t*(1.0e0/(1.0e0-r)-r*w)+w1;
    return rlog1_var;
S40:
    w = *x+0.5e0+0.5e0;
    rlog1_var = *x-log(w);
...
static int K3 = 9;
static int K4 = 10;
static double spmpar_var,b,binv,bm1,one,w,z;
...
static int K1 = 5;
static double stvaln_var,sign,y,z;
/*
     ..
     .. Executable Statements ..
*/
...
S20:
    y = sqrt(-(2.0e0*log(z)));
    stvaln_var = y+devlpl(xnum,&K1,&y)/devlpl(xden,&K1,&y);
    stvaln_var = sign*stvaln_var;
    return stvaln_var;
}
/************************************************************************
FIFDINT:
