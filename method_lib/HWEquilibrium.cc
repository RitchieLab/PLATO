...
void HWEquilibrium::doFilter(Marker* mark){
	if(options.doThreshMarkersLow() || options.doThreshMarkersHigh()){
		bool inc = false;
		if(options.doThreshMarkersLow() && ((dLess(hw_O, options.getThreshMarkersLow()) && hw_O != -1))){// || (dLess(hw_C,options.getThreshMarkersLow()) && hw_C != -1))){
			mark->setEnabled(false);
			inc = true;
		}
		if(options.doThreshMarkersHigh() && ((dGreater(hw_O, options.getThreshMarkersHigh()) && hw_O != -1))){// || (dGreater(hw_C, options.getThreshMarkersHigh()) && hw_C != -1))){
			mark->setEnabled(false);
			inc = true;
		}
		if(inc){
...
void HWEquilibrium::processHWEPT(){
	...
	for(int i = 0; i < msize; i++){
		...
		if(mark->isEnabled() && isValidMarker(mark, &options, prev_base, prev_chrom)){
			...
			pvals << mark->toString() << "\t" << ca_pval << "\t" << con_pval << "\t" << geno_pval << "\t" << allele_p << "\t" << hw_Ca << "\t" << hw_Con << endl;
//			doFilterHWEPT(mark);
		}
	}
	pvals.close();
