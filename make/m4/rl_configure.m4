...
AC_LANG_PUSH(C++)
AX_CHECK_LINK_FLAG([-rdynamic], [LDFLAGS="$LDFLAGS -rdynamic"])
AC_LANG_POP()
...
AX_BOOST_LIB([thread],[boost::thread_group thrds;],[$BOOST_SYSTEM_LIB])
...
AX_BOOST_LIB([iostreams],[boost::iostreams::zlib_decompressor();],[-lz],[boost/iostreams/filter/zlib.hpp])
...
if test "x$3" != "x"; then
	...
	if test "x$PYTHON" = "x"; then
		...
		AX_PROG_PYTHON_VERSION([$3],[rl_found_python="yes"],[rl_found_python="no"])
...
if test "$ac_cv_c_inline" != no ; then
  AC_DEFINE([HAVE_INLINE],[1],[I have inline!])
