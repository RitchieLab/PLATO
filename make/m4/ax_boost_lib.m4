...
[
	...
	AC_ARG_WITH([boost-]m4_bpatsubst($1,[_],[-]),
	...
	if test "x$want_lib" = "xyes"; then
		...
		#AC_COMPILE_IFELSE(AC_LANG_PROGRAM([[@%:@include <$HDR_INCLUDE> @%:@include <iostream>]], [[$2]]),
...
#include <$HDR_INCLUDE> 
		...
		if test "x$lib_compile" != "xno"; then
			...
			LIB_LIST=`ls $BOOSTLIBDIR/*boost_$1* 2>/dev/null | sed 's/.*\///g' | sed 's/\..*//g' | sed 's/^lib//g' | sort -u`
			if test "x$ax_boost_lib" != "x"; then
				LIB_LIST="$ax_boost_lib"
			fi
			
			LIB_FINAL=""
			
			for lib_cand in $LIB_LIST; do
				LDFLAGS="$LDFLAGS_SAVE $BOOST_LDFLAGS"
				export LDFLAGS
				LIBS="$3 -l$lib_cand $LIBS_SAVE"
				export LIBS
				
				# NOTE: every boost library has an _init function
				#AC_CHECK_LIB($lib_cand, main, [lib_link=yes],[lib_link=no], [$3])
