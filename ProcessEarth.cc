...
//
//	//cout << "options.getUseLoci().size(): " + getString<int>(options.getUseLoci().size()) + "\n";
//	//cout << "ds->get_markers()->begin() = " << getString<int>(ds->get_markers()->begin()) << "\n";
...
//			Marker* mark = *iter;
//			int snploc = iter - ds->get_markers()->begin();	//????? not sure what is going on here.  ?distance in bp from the first marker in the dataset to the current one?
//			cout << "snploc= " << getString<int>(snploc) << "\n";
