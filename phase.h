...
class HaploPhase
{
 public:
  ...
  HaploPhase(vector<Marker*>* markers_, vector<Sample*>* samples_) :
    samples(samples_), markers(markers)_
    {
      ambig.resize(samples->size(), false);
      include.resize(samples->size(), true);
      pp.resize(samples->size());
      hap1.resize(samples->size());
      hap2.resize(samples->size());
      X=haploid = false;
      subhaplotypes = false;
  ...
  void makeSlidingWindow(int,int);
  ...
  void reset()
	...
	hap[i].clear();
      ...
      hap1.resize(samples->size());
      hap2.resize(samples->size());
      ambig.resize(samples->size());
      include.resize(samples->size());
      subhaplotypes = false;
      downcoding.clear();
      calculateDp = false;
